<template>
  <div>
    ...
    <div class="mb-5">
      <pelaksanaan-navbar active="catatan" :has-permission="hasPermission"/>
      ...
      <v-card elevation="4" class="rounded-lg">
        ...
        <v-card-text>
          <v-textarea class="mb-3 rounded-lg" color="blue" outlined auto-grow name="input-7-4" label="Tulis catatan" rows="3"
            hide-details="auto" v-model="create_data"></v-textarea>
          <div class="d-flex justify-end">
            <v-btn elevation="0" dark color="light-blue" class="rounded-lg" @click="createSave">Simpan</v-btn>
          </div>
        </v-card-text>
      </v-card>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        this.$axios.get(`/v1/pelaksanaan/${this.id}/catatan`)
          .then(res => {
      ...
      createSave() {
        ...
        this.$axios.post(`/v1/pelaksanaan/${this.id}/catatan`, data)
          .then(res => {
