<template>
  <div v-if="hasPermission">
    ...
    <div class="mb-5">
      <pelaksanaan-navbar active="hapus" :has-permission="hasPermission"/>

      <v-card elevation="4" class="rounded-lg">
        <v-card-text style="max-height: 500px; overflow-y: auto">
          Hapus project? Data yang telah dihapus tidak dapat dikembalikan.
          <div class="d-flex justify-end">
            <v-btn elevation="0" dark color="red" class="rounded-lg" @click="delete_dialog = true">Hapus</v-btn>
          </div>
        </v-card-text>
      </v-card>
    </div>
    ...
    <v-dialog v-model="delete_dialog" persistent max-width="400">
      <v-card class="rounded-lg">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="light" text @click="delete_dialog = false">
            Batal
          </v-btn>
