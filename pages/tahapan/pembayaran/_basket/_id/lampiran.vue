<template>
  <div>
    <detail-lampiran-dialog :dialog="detail_dialog" :file="detail_data" tahap="pembayaran" @close="closeDetailDialog"
      @reload="getLampiran" />
    ...
    <div class="mb-5">
      <pembayaran-navbar active="lampiran" :has-permission="hasPermission"/>

      <v-card elevation="4" class="rounded-lg">
        <v-card-title class="mb-4">
          <v-text-field style="max-width: 300px" v-model="search" append-icon="mdi-magnify" label="Cari" single-line
            hide-details v-debounce="getLampiran" class="rounded-lg" dense outlined></v-text-field>
          <v-spacer></v-spacer>
          <upload-dialog tahap="pembayaran" @reload="getLampiran" :has-permission="hasPermission"/>
        </v-card-title>
        ...
        <v-card-text>
          ...
          <div class="d-flex justify-center mt-5">
            <v-btn elevation="0" dark class="rounded-lg" color="light-blue" @click="nextPage">Muat lebih banyak</v-btn>
          </div>
        </v-card-text>
      </v-card>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getLampiran() {
        this.$axios.get(`/v1/pembayaran/${this.$route.params.id}/lampiran?search=${this.search}&page=${this.page}`)
          .then(res => {
            if (this.page == 1) {
