<template>
  <div>
    ...
    <div class="mb-5">
      <skki-navbar active="lampiran" :has-permission="hasPermission"/>
      ...
      <v-card elevation="4" class="rounded-lg">
        <v-card-title class="mb-4">
          ...
          ></v-text-field>
          <v-spacer></v-spacer>
          <upload-dialog tahap="skki" @reload="getLampiran" :has-permission="hasPermission"/>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    getLampiran() {
      this.$axios.get(`/v1/skki/${this.$route.params.id}/lampiran?search=${this.search}&page=${this.page}`)
        .then(res => {
          if(this.page == 1) {
